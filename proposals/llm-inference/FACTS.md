...
## Self-speculation and draft heads (Chapter 30)

| Fact | Value | Source | Checked |
|---|---|---|---|
| Medusa, the paper | Tianle Cai, Yuhong Li, Zhengyang Geng, Hongwu Peng, Jason D. Lee, Deming Chen, Tri Dao, "Medusa: Simple LLM Inference Acceleration Framework with Multiple Decoding Heads", arXiv:2401.10774, ICML 2024 | arXiv abstract | 2026-09-21 |
| What it does | "adding extra decoding heads to predict multiple subsequent tokens in parallel"; "using a tree-based attention mechanism, Medusa constructs multiple candidate continuations and verifies them simultaneously in each decoding step" | arXiv:2401.10774, abstract | 2026-09-21 |
| Its speedups | "Medusa-1 can achieve over 2.2x speedup without compromising generation quality, while Medusa-2 further improves the speedup to 2.3-3.6x" | same | 2026-09-21 |
| A Medusa head's architecture | `nn.Sequential(*([ResBlock(hidden_size)] * medusa_num_layers), nn.Linear(hidden_size, vocab_size, bias=False))`, one per head; `ResBlock` is `x + SiLU(Linear(hidden_size, hidden_size)(x))`. **Each head carries its own projection to the full vocabulary.** | github.com/FasterDecoding/Medusa, medusa/model/medusa_model.py | 2026-09-21 |
| Its defaults | `--medusa_num_heads 3 --medusa_num_layers 1` in the project's own training command | Medusa README | 2026-09-21 |
| EAGLE, the paper | Yuhui Li, Fangyun Wei, Chao Zhang, Hongyang Zhang, "EAGLE: Speculative Sampling Requires Rethinking Feature Uncertainty", arXiv:2401.15077, ICML 2024 | arXiv abstract | 2026-09-21 |
| What it does | "autoregression at the feature (second-to-top-layer) level is more straightforward than at the token level"; resolves the resulting uncertainty by "incorporating a token sequence advanced by one time step", "with minimal overhead" | arXiv:2401.15077, abstract | 2026-09-21 |
| Its speedup | "For LLaMA2-Chat 70B, EAGLE achieved a latency speedup ratio of 2.7x-3.5x, doubled throughput" | same | 2026-09-21 |
| EAGLE draft sizes | published per target model: Vicuna-7B 0.24B, Vicuna-13B 0.37B, Vicuna-33B 0.56B, LLaMA2-Chat 70B / LLaMA3-Instruct 70B 0.99B, Qwen2-72B-Instruct 1.05B, Mixtral-8x7B-Instruct 0.28B. The README does **not** state the layer count or whether the embedding and output layer are shared | github.com/SafeAILab/EAGLE README | 2026-09-21 |
| Prompt lookup decoding | Apoorv Saxena, "Prompt Lookup Decoding", November 2023 -- replaces the draft model with string matching in the prompt | github.com/apoorvumang/prompt-lookup-decoding | 2026-09-21 |
| Its claim | "significant speedups (2x-4x) in input-grounded tasks, with no effect on output quality"; on summarisation and context-QA, "a relatively consistent 2.4x speedup (on average)" | same README | 2026-09-21 |
