...
**Depends on:** {{ch:arithmetic-intensity}}.
...
{{ch:arithmetic-intensity}} gave you a way to predict what is possible.
...
**{{spread}}** separates {{flattering}} tokens per second from
{{damning}}, on one machine, with one unchanged program.
...
faults in pages and finds nothing in cache. Reporting it costs {{cold_penalty}} here.
*Fix:* the harness always runs the work before it starts timing, and
never reports a cold result.

**2. A single run.** The luckiest of {{samples}} runs is
{{best_run_gain}} the median. Run a benchmark once and you have
...
experience, as {{ch:latency-throughput}} showed. *Fix:* percentiles.
The harness reports p50 and p99 and never a bare mean.

**4. Unstated conditions.** The same system reports {{honest}} tokens
per second at a {{short}}-token context and {{context_penalty}} that at
{{long}}. Both are "decode throughput". *Fix:* every result carries the
...
so — {{ch:the-kv-cache}} reports its headline speedup as "about 42x"
for exactly this reason, and {{ch:the-memory-wall}} quotes a round
...
service. {{ch:capacity-planning}} needs the second, and exercise 9.5
builds it.
...
therefore a best case, and {{ch:observability}} deals with measuring a
...
| Quantity | Value |
|---|---|
| Spread across honest framings, unchanged system | {{spread}} |
| Cost of reporting a cold run | {{cold_penalty}} |
| Gain from reporting the luckiest of {{samples}} runs | {{best_run_gain}} |
| Cost of honest percentiles (p99 against median) | {{tail_penalty}} |
| Cost of a {{long}}-token context against {{short}} | {{context_penalty}} |
