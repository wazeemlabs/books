...
tidiness. It is the seam that lets {{ch:paged-attention}} replace the
...
{{seqs_8k}} sequences of 8K tokens — before any **fragmentation**,
...
- **Eviction and preemption.** <!-- defines: eviction, preemption -->
  ...
  are worked on next and which wait. {{ch:continuous-batching}} builds
  one; {{ch:chunked-prefill}} gives it a policy.
