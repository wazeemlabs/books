...
{{ch:the-memory-wall}} ended with the only cure for decoding's problem:
...
Here is the distribution of {{samples}} consecutive decode steps on
...
The mean is {{mean_ratio}} the median here, which is the one case where
an average is not actively misleading — and even so it tells you
nothing about the shape. The p99 is {{tail_ratio}} the median, and the
slowest step of the {{samples}} took {{max_ms}} against a typical
{{p50_ms}}.
...
Serving one sequence gives {{batch1_tps}} tokens per second at
{{batch1_itl}} between words. Serving {{batchmax}} gives
{{batchmax_tps}} at {{batchmax_itl}}. **Throughput rises
{{throughput_gain}}; the wait between words rises {{latency_cost}}.**
...
**Only the {{binding_budget}} budget actually binds.** Every looser
...
{{ch:where-the-memory-goes}} computes.
...
- **Traffic** — {{prompt}}-token conversations, {{output}}-token
  replies, 200 requests a second at the busy hour.
- **The promise** — p99 time to first token under {{ttft_allowed}},
  p99 between words under {{case_itl}}, available 99.9% of the time.
...
of **{{case_batch}} concurrent sequences**, {{case_achieved_itl}}
between words, {{case_tps}} tokens per second.

And note what it says about TTFT. Prefill of a {{prompt}}-token prompt
takes about **{{ttft_ms}}** against the {{ttft_allowed}} promised —
{{ttft_headroom}} of headroom. For this workload, time to first token
...
{{words_per_s}} a second — several times faster than anyone reads — and
the complete {{output}}-token reply lands in {{reply_s}}. That is the
...
{{ch:batching}} and {{ch:continuous-batching}} measure the real curve,
and it is lower.
...
| Batching's trade, this model | {{throughput_gain}} throughput for {{latency_cost}} latency |
| Measured tail, idle machine | p99 is {{tail_ratio}} the median — and that is the floor |
| The case study's operating point | {{case_batch}} sequences, {{case_achieved_itl}} between words |
...
**★★ 5.3** The chapter says only the {{binding_budget}} budget binds,
...
the operating point? ({{ch:fp8}} is one way to halve it.)

**★★ 5.4** A reply is {{output}} tokens, so it is {{output}} decode
