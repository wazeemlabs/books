...
5. Explain why a 7.4x in a paper and a {{co_gain_pct}} loss in this
...
**Colocated wins: {{co_tok}} tokens a second against {{best_tok}},
{{co_over_best}}.** It also has a better wait for a first token
({{co_ttft99}} against {{best_ttft99}} at the p99) and a better median
wait between tokens ({{co_itl50}} against {{best_itl50}}). The
disaggregated fleet is very slightly ahead on the between-token p99
({{best_itl99}} against {{co_itl99}}) and that is all it wins.

Three per cent sounds like a rounding error, and at this particular
load it is not one. The traffic asks for {{offered_sampled}} tokens a
second. The colocated fleet produces {{co_of_offered}} of that and
{{co_keeps}}; the best split produces {{best_of_offered}} and
{{best_keeps}}. Three per cent of {{fleet}} machines is a thirteenth
...
| Disaggregated against colocated, same fleet | {{best_tok}} against {{co_tok}} — colocated by {{co_gain_pct}} |
...
{{ch:chunked-prefill}}'s iterations do. How much of the
{{co_gain_pct}} gap does that close, and why does it close less than
you expected?
