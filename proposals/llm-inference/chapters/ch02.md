...
**Depends on:** {{ch:the-cost-of-a-token}}.
...
{{ch:the-cost-of-a-token}} made a claim and asked you to accept it:
...
We will follow a five-word prompt — **"{{prompt}}"** — through a real
...
> says a token becomes "a vector of {{d_model}} numbers", it means the
> word has been replaced by a list of {{d_model}} numbers that the model
> can do arithmetic on.
>
> A **parameter** (or *weight*) is one number that was learned during
> training and is now fixed. Our model has {{params}} of them. A
...
model's {{n_layers}} layers; the model scores every word it knows and
...
Our model has a deliberately tiny vocabulary of {{vocab}} whole words,
...
(typically 30,000–200,000 rather than {{vocab}}) but changes nothing
...
vectors of {{d_model}} numbers each:
...
{{d_model}} numbers, and each layer lets it revise its notepad. The
model's answer is whatever is written on the last token's notepad at
the end.

## Step 2: what a layer does

Each of the {{n_layers}} layers does two things in order.
...
Those five numbers add to {{row_sum}} — attention always divides a
fixed budget of 1.0, it never creates more. The largest share,
{{row_biggest}}, went to "{{row_biggest_word}}". And the row simply
...
In our model, **{{ffn_share}}** of each layer's parameters sit in these
two feed-forward tables rather than in attention, and the proportion is
similar in production models. When {{ch:the-cost-of-a-token}} said the
...
Those {{vocab}} raw scores are turned into probabilities that sum to 1:

![The ten highest-scoring next words](code/figures/ch02-scores.svg)

**Figure 2.3** — The ten best of {{vocab}} possible next words.
*Provenance in `code/figures/ch02-scores.caption.txt`.*

The model gives "{{chosen}}" {{chosen_prob}}. Pure chance would give
every word {{uniform_prob}}, and again — this model is untrained, so
...
Read the right-hand column against {{ch:the-cost-of-a-token}}'s
arithmetic: {{ref_gb}} read per token, at an accelerator's
memory bandwidth, is the {{ref_floor_ms}} that set the floor on how
fast one user can be served. {{ch:the-memory-wall}} takes this fact
apart properly.

Note also how lopsided the last two rows are. Producing a token needs
{{ref_flops}} arithmetic operations — which a modern accelerator can do
in a fraction of a millisecond — but requires reading {{ref_gb}}, which
...
That asymmetry is worth {{ref_kv}} per token of storage in a production
...
{{ch:naive-generate-loop}} recomputes them and measures the damage.
{{ch:the-kv-cache}} keeps them, and that single decision is the largest
...
- **How the model knows the order of the words.** Something must
  ...
  matters for long contexts, and {{ch:long-context}} covers it.
...
- **Multiple heads.** Attention does not run once per layer; it runs
  ...
  values, and the results are joined. Our model has {{n_heads}} such
  heads of {{head_dim}} numbers each. Figure 2.2 shows one of them. This
  matters a great deal for memory, and {{ch:the-kv-cache}} returns to it.
- **Models that do not use all their weights.** Some large models
  ...
  fact one in an interesting way, and {{ch:long-context}} deals with it.
...
- the vocabulary grows from {{vocab}} to 30,000–200,000 sub-word pieces;
- {{d_model}} becomes 4,096 or more, and {{n_layers}} becomes 32 to 80;
- attention uses fewer key/value heads than query heads, to cut that
  {{ref_kv}} per token — the reason is now obvious to you, and
...
| Weights read per token, production 8B | {{ref_gb}} |
| Arithmetic per token, production 8B | {{ref_flops}} operations |
| Keys and values stored per token, production 8B | {{ref_kv}} |
...
**★ 2.1** Our prompt is {{n_tokens}} tokens. If the model writes a
