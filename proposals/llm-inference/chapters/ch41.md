...
from {{idle_rate}} requests a second to {{hot_rate}} — {{hot_util}} of
capacity — the end-to-end p99 goes from {{idle_p99}} to {{hot_p99}},
...
Every rate was run twice: once over {{requests}} requests and once
over {{requests_long}}. The column is how far the furthest of the
mean, the p99 and the throughput moved between the two runs. Up to
{{settled_to}} requests a second it moves by {{drift_settled}} or
less. From {{unsettled_from}} it does not: at {{top_rate}} a second
the p99 is {{p99_unsettled_short}} over the shorter run and
{{p99_unsettled_long}} over the longer one. It doubled because the run
doubled.
...
{{little_breaks_at}} requests a second, which is exactly the load at
