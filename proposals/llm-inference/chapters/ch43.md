...
{{ch:measuring-honestly}}, {{ch:capacity-planning}}.
...
first-token p99 is {{here_ttft_true}} and the dashboard says
{{here_ttft_shown}}, which is close enough. The worst case measured is
not: {{worst_metric}} at {{worst_rate}} requests a second is
{{worst_true}} in the trace and **{{worst_shown}}** on the screen,
{{worst_error}}.
...
show it. Between {{jump_from_rate}} and {{jump_to_rate}} requests a
second the real p99 {{jump_metric}} goes from {{jump_true_from}} to
{{jump_true_to}} — **{{jump_true_pct}}**, a nothing. The dashboard
goes from {{jump_shown_from}} to {{jump_shown_to}} —
**{{jump_shown_pct}}**. The value crossed a bucket edge, from
{{jump_bucket_from}} into {{jump_bucket_to}}, and the reported number
stepped the width of the new bucket.

**A bucketed percentile does not drift, it jumps**, and it amplified
this change {{jump_amplification}} times. Somebody is going to open
...
case study promises {{itl_budget}} between tokens, and vLLM's buckets
happen to include an edge at exactly that: the bucket containing the
promise runs {{itl_budget_bucket}}. So the number of samples at or
below {{itl_budget}} is known precisely, and so is the share:
...
alerted on the promise. The first-token promise of {{ttft_budget}}
sits on an edge too — its bucket is {{ttft_budget_bucket}} — so the
...
Here is the same traffic with the block pool cut to {{fault_share}} of
what {{ch:capacity-planning}} sized — {{fault_gb}}, which is what a
...
**`num_requests_waiting` goes from {{waiting_healthy}} to
{{waiting_faulty}}.** A factor of {{waiting_moved}}. This is the
signal. It is a gauge, it is free, and it is the first thing to put on
a dashboard.

**`num_requests_running` goes from {{running_healthy}} *down* to
{{running_faulty}}.** The server is preempting: sequences are being
...
**The first-token p99 goes from {{ttft_healthy}} to {{ttft_faulty}}**
— {{ttft_moved}}. This is the user-visible damage, and it is a
consequence rather than a cause: by the time it moves, the queue has
already been building.

**The between-tokens p99 goes from {{itl_healthy}} to
{{itl_faulty}}.** {{itl_moved}}. It does not move. The requests that
...
And `num_preemptions` goes from {{preempt_healthy}} to
{{preempt_faulty}}. It was exactly zero before. A counter that is
...
A {{short_incident}}-second incident is caught {{short_caught_15}} of
the time by a 15-second scrape and **{{short_caught_60}}** of the time
...
{{tiny_caught_15}} of the time even at 15 seconds.
...
{{one_histogram}} series before a single label is attached —
{{buckets_itl}} buckets, a sum and a count. Attach {{labels}} labels
of the sizes in that table and one histogram is
**{{histogram_series}} series**, against {{gauge_series}} for a plain
...
One histogram with four labels is {{histogram_series}} series. Put
...
- **{{jump_true_pct}} against {{jump_shown_pct}}** — the same change
...
- **{{worst_true}} against {{worst_shown}}** — the same p99, in the
  trace and on the dashboard.
- **{{waiting_moved}}** — how far `num_requests_waiting` moves in a
  memory incident, while `num_requests_running` goes *down*, from
  {{running_healthy}} to {{running_faulty}}.
- **{{itl_moved}}** — how far the between-tokens p99 moves in the same
  incident. A service can be broken with that promise intact.
- **{{preempt_healthy}} to {{preempt_faulty}}** — a counter that is
  normally zero, which is the easiest alert there is.
- **{{short_caught_60}}** — how often a {{short_incident}}-second
  incident is visible on a minute-scraped dashboard.
- **{{histogram_series}} series** — one histogram with {{labels}}
...
scheduler of {{ch:chunked-prefill}}: which spans, which attributes,
