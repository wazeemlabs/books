...
still is not enough: throughput falls to {{small_tok}}, against the
{{offered_tok}} the traffic is offering. A server producing less than
it is asked for has a queue that grows for as long as the traffic
lasts, so the {{small_ttft99}} this run reports for the 99th
percentile wait is a number about how long the run was rather than
about the server — {{ch:capacity-planning}} shows how to tell.
...
{{large_itl99}} — better than the previous chapter's {{base_itl99}},
because the prompt now rides along with the decodes instead of
displacing them, and over the {{itl_budget}} promise all the same. A
budget nothing ever exceeds is not a budget.

**And the middle is narrow.** Exactly {{passing_count}} of the
budgets keep both promises at this load, {{passing_low}} to
{{passing_high}}; by {{first_failing_high}} the between-token p99 has
crossed the promise, and {{chosen}} is the one of the three that
...
- **Sweep the budget at the load you are worried about, not at an easy
  one.** At {{rate}} requests a second in our sweep, {{low_passing_count}}
  of the budgets passed and only {{low_failing}} failed. At
  {{rate_high}} the passing range narrows to {{passing_count}}, and
