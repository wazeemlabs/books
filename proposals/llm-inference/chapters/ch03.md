...
**{{intensity_ratio}} is a ratio of work per byte. It is not a ratio of
...
this: prefill costs **{{prefill_per_token_us}} microseconds** per token
of prompt, decode costs **{{decode_ms}}** per token of output. That is
**{{time_ratio}}** — not {{intensity_ratio}}. Keep the two apart.
...
{{time_ratio}}. There are two reasons, and {{ch:the-memory-wall}}
...
accelerator's {{ridge}} operations per byte — it has proportionally far
...
| **Paging** the cache ({{ch:paged-attention}}) — storing it in fixed-size pieces that can live anywhere | decode | fits more sequences, so more work shares each fetch | <!-- defines: paging -->
...
| **Prefix caching** ({{ch:prefix-caching}}) — a prompt's **prefix** is however much of its start another prompt also had | prefill | skips prompt work already done for someone else | <!-- defines: prefix, prefix caching -->
| Attention kernels ({{ch:attention-kernels}}) — a **kernel** is one program that runs on the accelerator | prefill mostly | prefill is where attention's cost concentrates | <!-- defines: kernel -->
| **Chunked prefill** ({{ch:chunked-prefill}}) — reading a long prompt a few hundred tokens at a time rather than all at once | both, by arbitrating | stops a long prompt stalling everyone's decode | <!-- defines: chunked prefill -->
...
{{ch:the-kv-cache}} does the arithmetic.
...
({{intensity_ratio}}) and time per token ({{time_ratio}}). Derive the
...
per byte instead of {{ridge}}, and check your prediction against
{{ch:the-memory-wall}}'s measurements.
